...
export function AuthStatus() {
  ...
  if (user) {
    ...
    return (
      <div className="flex items-center gap-2">
        <Link href="/profile" className="flex items-center gap-2 rounded-full border border-[var(--border)] bg-slate-50 px-2 py-1.5 text-sm font-medium text-[var(--foreground)] transition hover:border-blue-600 hover:bg-blue-600 hover:text-white">
          {avatar ? (
            <span aria-label="Profile picture" role="img" className="h-7 w-7 rounded-full bg-cover bg-center" style={{ backgroundImage: `url(${avatar})` }} />
          ) : (
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-600 text-[10px] font-bold text-white">{initials}</span>
          )}
          <span className="hidden sm:inline">{user.name.split(" ")[0]}</span>
        </Link>
      </div>
  ...
  return (
    <Link href="/login" className="rounded-full border border-[var(--border)] bg-white px-4 py-2 text-sm font-semibold text-[var(--foreground)] transition hover:border-blue-600 hover:bg-blue-600 hover:text-white">
      Sign in
    </Link>
  );
}