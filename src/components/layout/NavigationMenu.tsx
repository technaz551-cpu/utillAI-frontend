...
========================================================= */
const TOOLS_PER_CATEGORY_LIMIT = 4;
...
export function NavigationMenu({
  ...
  /*
   * Tracks which category cards (inside the Tools popup)
   * currently have "Show more" expanded, by category slug.
   */
  const [expandedToolCategories, setExpandedToolCategories] =
    useState<Set<string>>(new Set());

  /*
   * Tracks whether the Categories popup list is expanded
   * to show everything, or just the first few.
   */
  ...
  function toggleMenu(
    ...
    // Reset "show more" state whenever a popup is (re)opened,
  ...
  /* =========================================================
     TOGGLE "SHOW MORE" FOR A SINGLE CATEGORY'S TOOL LIST
  ========================================================= */

  function toggleToolCategoryExpanded(
    categorySlug: string
  ) {
    setExpandedToolCategories((current) => {
      ...
      {navItems.map((item) => {
                        ...
                        {categories.map(
                          (category) => {
                            ...
                            const visibleTools =
                              isExpanded
                                ? category.tools
                                : category.tools.slice(
                                    0,
                                    TOOLS_PER_CATEGORY_LIMIT
                                  );
                              ...
                              >
                                ...
                                {category.tools
                                  ...
                                  <>
                                    ...
                                    {/* Show more / less toggle for this category's tools */}
                                    {category.tools.length >
                                      TOOLS_PER_CATEGORY_LIMIT && (
                                      <button
                                        type="button"
                                        onClick={() =>
                                          toggleToolCategoryExpanded(
                                            category.slug
                                          )
                                        }
                                        className="mt-1.5 pl-4 text-[11px] font-semibold text-[#1769E0] transition hover:underline"
                                      >
                                        {isExpanded
                                          ? "Show less"
        ...
        if (item.label === "Categories") {
          const visibleCategories = showAllCategories
            ? categories
            : categories.slice(0, CATEGORIES_LIMIT);
                ...
                <div className="absolute left-1/2 top-full z-[100] mt-2 w-80 max-w-[calc(100vw-40px)] -translate-x-1/2 rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_20px_60px_rgba(15,23,42,0.16)]">
                  ...
                  {categories.length > 0 ? (
                    <div className="max-h-[400px] overflow-y-auto pr-2">
                      ...
                      {/* Show more / less toggle for the categories list */}
                      {categories.length >
                        CATEGORIES_LIMIT && (
                        <button
                          type="button"
                          ...
                          className="mt-2 w-full rounded-xl px-3 py-2 text-center text-xs font-semibold text-[#1769E0] transition hover:bg-[#F0F5FF]"
                        >
                          {showAllCategories
                            ? "Show less"
