...
export default function LoginPage() {
  ...
  return (
    <main className="section-shell flex items-center justify-center py-16 md:py-24">
      <div className="w-full max-w-md rounded-[32px] border border-[var(--border)] bg-white p-8 shadow-xl">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-600">Welcome back</p>
        <h1 className="mt-3 text-3xl font-black tracking-[-0.06em] text-[var(--foreground)]">Login</h1>
        <p className="mt-2 text-sm text-slate-500">Access your toolkit and saved workflows.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          <label className="block text-sm font-medium text-[var(--foreground)]">
            ...
            <input
              ...
              onChange={(event) => setEmail(event.target.value)}
              placeholder="you@example.com"
              className="mt-2 w-full rounded-2xl border border-[var(--border)] bg-slate-50 px-4 py-3 outline-none transition focus:border-blue-600"
            />
          </label>
        ...
        <p className="mt-6 text-center text-sm text-slate-500">
          Don’t have an account? <Link href="/register" className="font-semibold text-blue-600">Sign up</Link>
        </p>
      </div>
    </main>
  );
